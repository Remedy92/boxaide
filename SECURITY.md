...
| Asset | Protection |
|---|---|
| Mail passwords and OAuth tokens | AES-256-GCM at rest, per-record random nonce, authenticated. The master key lives in your data directory, never in the database. |
...
| `/api/local-bootstrap` | Hands out the token, so it is gated on both the `Origin` and the `Host` header being loopback, sends `Cache-Control: no-store`, and carries no CORS headers. |
