...
**No.** A non-developer cannot get Superhuman / Apple Mail / Outlook-class “Sign in with Google” UX into a product like Boxaide (formerly Mailmux) without **someone else** owning a production OAuth client (and, for Gmail restricted mail scopes, Google verification + usually CASA).

| What the layperson does | Who paid the verification cost | Works for strangers? |
| --- | --- | --- |
| App password + IMAP form (Boxaide today) | Nobody | Partially — fails often (2SV, Workspace policy, Advanced Protection) |
| “Sign in with Google” using **Boxaide’s** OAuth client | Boxaide project (or a paid proxy that already verified) | Yes, after Google production verification |
...
Boxaide today is the second column for Gmail/Outlook personal accounts (app passwords). That is fine for developers and Fastmail users. It is not consumer UX.
...
| B. Boxaide-owned OAuth client (desktop/public + PKCE or hosted relay) | Good after verification | No for end user | Yes for core | **High** (Google restricted; Microsoft lighter) | Partial — Gmail may need Gmail API |
...
- Requires a Gmail API client in Boxaide (not only ImapFlow). That is a second mail backend for Gmail, not a small auth tweak.
...
### Scope classes that matter for Boxaide
...
- **Boxaide MCP agents** that send full mailbox content to arbitrary third-party LLMs need careful product design and privacy-policy wording. Risk of verification rejection or later suspension if “agent” implies unrestricted export of mail to external model trainers.
...
2. Public client for local Boxaide; confidential client only if you run a real backend.
...
**Tension:** “Sign in with Google for any stranger who `npm install`s Boxaide” **requires** a project-owned verified client (Path B). That is operational cost on the **maintainer**, not on the user. It does not force a paid SaaS into the data path if tokens stay local.
...
2. **Path B — Boxaide-owned OAuth (required substrate for D)**  
...
User clicks Connect Gmail
        │
        ▼
Boxaide (local) generates PKCE verifier/challenge
opens system browser → Google authorize URL
  client_id = Boxaide production desktop client
...
exchanges code + code_verifier → access_token + refresh_token
        │
        ▼
Store refresh_token with existing AES-GCM secret store (~/.boxaide)
...
**What the Boxaide project must do:**
...
6. Optional: allow override env `BOXAIDE_GOOGLE_CLIENT_ID` for BYO / Internal Workspace apps.

**Hosted auth relay variant (usually worse for Boxaide):** a small public HTTPS service that holds the confidential client secret and returns tokens to localhost. Adds ToS surface, availability dependency, and a server that sees refresh tokens — only justified if Desktop client type is rejected for your verification package (unlikely if you truthfully ship a local app).
