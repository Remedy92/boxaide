...
| **A runner blocked before spawn falls back; Antigravity with global MCPs is not an unattended runner** | A saved schedule should survive an uninstalled CLI or a configuration that became unsafe. The launcher records the reason and uses an installed isolated runner, preferring Codex. Antigravity always merges `~/.gemini/config/mcp_config.json` and offers no strict-config flag; loading Supabase or another unrelated MCP can fail before the Boxaide task starts, so that configuration remains available to watched chat but is excluded from schedules. |
