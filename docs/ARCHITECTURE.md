...
| Decision | Reason |
|---|---|
| **`output: "export"`, a static export with no server** | The same artefact is deployable to a static host *and* servable by the Node process. It also makes the privacy claim structural rather than a policy: with no route handlers, no server actions and no proxy, a host physically cannot see the token, the mail credentials or a message body. |
...
| **The driver's per-turn children are wrapped too** | Claude Code and Antigravity have no long-lived child. Their turns *are* the agent. `DriveOptions.command` carries the wrap so a driver cannot be the one spawn site that forgets. |
...
| **A run cannot read the user's agy MCP config** | agy has no strict-config flag and merges `~/.gemini/config/mcp_config.json` into every session, so the only way to keep an unattended run on Boxaide's own server is to take the file away. `AgentSpec.sandbox` is told whether the spawn is a chat or a run and denies a read of that one path for a run. `macosProfile` emits `confinement.deny` after every allow and seatbelt takes the last match, so the deny beats the `~/.gemini` write allowance the sign-in needs. A watched chat launch keeps the user's servers. This is the one isolation that depends on the sandbox being real: unconfined, the file loads and `GET /api/agents` says so rather than pretending. |
...
- Web: connect accounts, unified inbox, read, archive, delete (a move to Trash), move to folder, compose/send
...
| **A run keeps the agent it was given** | A blocked runner fails the run with its reason on the run row rather than being replaced. Swapping a saved runner for another CLI answers a question nobody asked: the person who chose Codex chose Codex, and a schedule that quietly ran on something else is a schedule whose output cannot be trusted or reproduced. A null `agent_id` still means "first available", resolved in registry order so it is the same answer every tick, and the agent and model that ran are written to the run row. `GET /api/agents` carries the reason each agent cannot chat or cannot run, so the block is visible before the run rather than only after it. |
...
| **Rail sections fold, and a folded header keeps its count** | One sidebar, not two. The chat list would otherwise push the mailboxes off the bottom. Folding buys that room, and the rule that makes it safe is that no signal can hide inside a closed section, so the CRM's approval count and the chat total stay on the header. The rail shows five chats whatever the history holds; the rest live in the all-chats dialog with the search box. |
...
| **Every child spawn and exit is written to `~/.boxaide/logs/boxaide.log`, not only to memory** | `launcher.lastExit` is one record and it dies with the process, so an agent that exited 1 with an empty stderr left nothing to read after a restart, and a stuck chat could not be diagnosed at all. The file is NDJSON, append-only, mode 0600, rotated at 5 MB keeping three files, and written by `src/log.ts` with no dependency and no reader in this process. It carries identifiers, exit codes, durations, driver events and the tail of a child's stderr. It never carries content: message bodies, mail, and tool results are absent by construction (a log field may only be a scalar), and credential shapes in a stderr tail are masked before the line lands. Logging is off until `configureLog` is given a data directory, so a `:memory:` install and every test writes nothing. |
