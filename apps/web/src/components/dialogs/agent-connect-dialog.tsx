      ...
      <DialogContent className="pane-scroll max-h-[86vh] max-w-[560px] overflow-y-auto">
        ...
        <div
          ...
          onKeyDown={(event) => {
            ...
            const next =
              targets[(Math.max(at, 0) + delta + targets.length) % targets.length];
            setActive(next.id);
        ...
        >
          ...
          <p className="text-[12px] leading-4 text-fg-tertiary">{target.note}</p>

          {target.carriesToken && (
            <p className="flex items-start gap-1.5 text-[12px] leading-4 text-warning">
              <StatusDot tone="warning" className="mt-1.5" />
              This snippet contains your access token. Don&rsquo;t paste it into
              anything public.
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          ...
          {/* User-triggered, never on load. A response proves this endpoint
              answers with this token — it never means an agent is attached, and
              this panel must not say so. */}
          <Button
            type="button"
            variant="secondary"
            size="sm"
