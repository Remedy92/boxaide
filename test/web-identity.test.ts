...
describe("web identity", () => {
  it("writes only sley.* settings keys", () => {
    ...
    expect(SETTINGS_EVENT).toBe("sley:settings");
    expect(LEGACY_TOKEN_KEY).toBe("mailmux_token");
  });

  it("names the provider-step app password Sley", () => {
    const steps = PROVIDER_PRESETS.flatMap((preset) => preset.steps).join("\n");
    expect(steps).toMatch(/Sley/);
    expect(steps).not.toMatch(/mailmux/i);
  });

  it("builds MCP snippets under mcpServers.sley and /sley.mcpb", () => {
    ...
    expect(desktop?.download).toEqual({
      href: "http://127.0.0.1:8787/sley.mcpb",
      filename: "sley.mcpb",
      action: "Download for Claude Desktop",
    });
    expect(desktop?.snippet).toContain('"sley"');
    expect(desktop?.snippet).toContain('"command": "sley"');
    expect(cursor?.snippet).toContain('"sley"');
  ...
  it("tells a refused origin to set SLEY_ALLOWED_ORIGINS", () => {
    expect(friendlyError("forbidden origin")).toContain("SLEY_ALLOWED_ORIGINS");
    expect(friendlyError("forbidden origin")).not.toMatch(/MAILMUX|mailmux/);
  });

  it("types HealthResponse.service as sley", () => {
    const types = readFileSync(join(WEB_SRC, "lib", "types.ts"), "utf8");
    expect(types).toContain('service: "sley"');
...
describe("settings fallback", () => {
  ...
  it("reads mailmux.* and mailmux_token when sley.* is absent", () => {
    installWindow();
